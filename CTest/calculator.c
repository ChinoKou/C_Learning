...
float function(float num1, float num2, char operator);
float deal();
...
float deal() {
    ...
    while (strchr(valid_operators, operator) == NULL) {
        printf("输入的运算符有误(不支持此运算符)，请重新输入运算符:\r\n");
        scanf(" %c", &operator);
        printf("\r\n");
    }

    if (strchr(sanjiao, operator) == NULL) {
        printf("请输入第二个数字:\r\n");
        scanf("%f", &num2);
    ...
    else if (strchr(sanjiao, operator) == NULL) {
        printf("%.2f %c %.2f = \n", num1, operator, num2);
    }
